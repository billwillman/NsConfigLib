...
//#if NET_STANDARD_2_0
    public static unsafe class SimdJsonN // 'N' stands for Native
    {
		...
		{
			return (uint)Global_jsonminify (jsonDataPtr, (size_t)jsonDataLength, output);
		}
    ...
    public unsafe class ParsedJsonN : IDisposable // 'N' stands for Native
    {
        ...
        public ParsedJsonN(void* handle)
		{ 
			this.Handle = handle;
		}
        public ParsedJsonN()
		{
			Handle = ParsedJson_ParsedJson();
		}

        public ParsedJsonIteratorN CreateIterator()
		{
			return new ParsedJsonIteratorN(this);
		}

        public bool AllocateCapacity(uint len, uint maxdepth = 1024) 
		{
			return ParsedJson_allocateCapacity(Handle, len, maxdepth) > 0;
		}
        ...
        public bool IsValid
		{
			get
			{
				return ParsedJson_isValid(Handle) > 0;
			}
		}

        public void Deallocate() 
		{
			ParsedJson_deallocate(Handle);
		}

        public void Init() 
		{
			ParsedJson_init(Handle);
		}

        public void WriteTape(uint64_t val, uint8_t c) 
		{
			ParsedJson_write_tape(Handle, val, c);
		}

        public void WriteTapeS64(int64_t i) 
		{
			ParsedJson_write_tape_s64(Handle, i);
		}

        public void WriteTapeDouble(double d) 
		{
			ParsedJson_write_tape_double(Handle, d);
		}

        public uint32_t GetCurrentLoc() 
		{
			return ParsedJson_get_current_loc(Handle);
		}

        public void AnnotatePreviousLoc(uint32_t savedLoc, uint64_t val)
		{
			ParsedJson_annotate_previousloc(Handle, savedLoc, val);
		}
    ...
    public unsafe class ParsedJsonIteratorN : IDisposable
    {
        ...
        public bool IsOk
		{
			get
			{
				return iterator_isOk(Handle) > 0;
			}
		}

        public uint TapeLocation {
			get {
				return (uint)iterator_get_tape_location (Handle);
			}
		}

		public uint TapeLength {
			get {
				return (uint)iterator_get_tape_length (Handle);
			}
		}

        public uint Depth {
			get {
				return (uint)iterator_get_depth (Handle);
			}
		}

        public uint8_t ScopeType {
			get {
				return iterator_get_scope_type (Handle);
			}
		}

        public bool MoveForward() 
		{
			return iterator_move_forward(Handle) > 0;
		}

        public uint8_t CurrentType {
			get {
				return iterator_get_type (Handle);
			}
		}

        public int64_t GetInteger()
		{
			return iterator_get_integer(Handle);
		}

        public bytechar* GetUtf8String()
		{
			return iterator_get_string(Handle);
		}

        public uint32_t GetUtf8StringLength() 
		{
			return iterator_get_string_length(Handle);
		}
		...
		{
			return iterator_get_double(Handle);
		}

        public bool IsObjectOrArray {
			get {
				return iterator_is_object_or_array (Handle) > 0;
			}
		}

        public bool IsObject {
			get {
				return iterator_is_object (Handle) > 0;
			}
		}

        public bool IsArray {
			get {
				return iterator_is_array (Handle) > 0;
			}
		}

        public bool IsString {
			get {
				return iterator_is_string (Handle) > 0;
			}
		}

        public bool IsInteger {
			get {
				return iterator_is_integer (Handle) > 0;
			}
		}
		...
		{
			get{
				return iterator_is_double (Handle) > 0;
			}
		}

        public bool MoveToKey(bytechar* key)
		{
			return iterator_move_to_key(Handle, key) > 0;
		}

        public bool Next()
		{
			return iterator_next(Handle) > 0;
		}

        public bool Prev()
		{
			return iterator_prev(Handle) > 0;
		}

        public bool Up()
		{
			return iterator_up(Handle) > 0;
		}

        public bool Down()
		{
			return iterator_down(Handle) > 0;
		}

        public void ToStartScope()
		{
			iterator_to_start_scope(Handle);
		}
