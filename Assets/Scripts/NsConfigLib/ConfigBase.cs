...
namespace NsLib.Config {
    public abstract class ConfigBase<KEY> {
        ...
        internal abstract KEY ReadKey(Stream stream);
        internal abstract bool WriteKey(Stream stream, KEY key);

        // 使用工具对继承的类进行反射，然后生成这部分代码，
        // 不使用反射来做（为了性能）
        // 默认情况使用反射来处理，后面类继承这个方法，工具生成这个代码
        // 后面一定要用工具生成代码，减少反射（因为是正式游戏运行时使用）
        internal virtual bool ReadValue(Stream stream) {
            if (stream == null)
                return false;
        ...
        internal bool WriteValue(Stream stream) {
            if (stream == null)
                return false;
    ...
    public abstract class ConfigStringKey: ConfigBase<string> {
        internal override string ReadKey(Stream stream) {
            if (stream == null)
    ...
    public abstract class ConfigUIntKey: ConfigBase<uint> {
        ...
        internal override bool WriteKey(Stream stream, uint key) {
